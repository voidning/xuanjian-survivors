...
function scenicGround(c){
 ...
 // Local ground colours connect each grove; the central court remains porcelain pale.
 for(const [x,y,w,col]of [[1050,1077,115,'#929b6540'],[1160,1031,83,'#b4966e38'],[2020,1052,122,'#78956c38'],[1834,852,65,'#ae86623b']]){path(c,`M${x-w} ${y}q${w*.4}-${18} ${w*.8}-${13}q${w*.6}-${16} ${w*1.2} 7q-${w*.3} 19-${w*.9} 14q-${w*.7} 8-${w*1.1}-${8}Z`,col,null);}
...
function oldPaving(c){
 c.save();
 // Whole-world base; road, water and the court are painted above this cached layer.
 const groves=[[1025,1090],[2020,1030],[2140,1510],[480,500],[2550,530],[550,1820],[2630,1850]];
 for(let row=0;row<Math.ceil(H/60)+1;row++){const yy=row*60-30,shift=row%2?48:0;
  for(let col=-1;col<Math.ceil(W/96)+1;col++){const xx=col*96+shift,cx=xx+48,cy=yy+30;
   const court=Math.max(0,1-Math.pow((cx-1570)/680,4)-Math.pow((cy-1030)/430,4));
   const shore=Math.min(Math.abs(cy-creekY(cx)),Math.abs(cy-creekY(cx)-127));
   const bank=.55+.45*Math.min(1,shore/90);
   let grove=1;for(const [gx,gy]of groves)grove=Math.min(grove,.6+.4*Math.min(1,Math.hypot((cx-gx)/180,(cy-gy)/120)));
   const n=((row*17+col*31)%13+13)%13,dx=n%3,dy=n%4;
   // A narrow value range and broken seams, not alternating dark/light checker squares.
   c.globalAlpha=(.15+court*.09)*bank*grove;
   shape(c,[[xx+2,yy+dy],[xx+94,yy+1],[xx+94,yy+58],[xx+3+dx,yy+58]],n%4===0?'#d8d3c5':n%4===1?'#e1dccf':'#e8e2d6');
   c.globalAlpha=(.11+court*.05)*bank*grove;
   if(n%3!==0)stroke(c,[[xx+8,yy+59],[xx+56+n,yy+58]],'#b7b3a2',.7);
   if(n%3===1)stroke(c,[[xx+94,yy+8],[xx+94,yy+36+n]],'#b8b5a7',.65);
   if(court>.3&&n===2)stroke(c,[[xx+6,yy+2],[xx+14,yy+8],[xx+11,yy+14]],'#b7b1a0',.65);
  }
 }
 c.restore();
}
function build(){cacheBuilds++;seed=73051;painting=document.createElement('canvas');painting.width=W;painting.height=H;const c=painting.getContext('2d');c.fillStyle=P.paper;c.fillRect(0,0,W,H);
 ...
 for(const [x,y,rx,ry,tint]of [[1030,1030,280,160,"#a4aa72"],[1180,989,180,100,"#b59b70"],[2040,1040,270,180,"#8eaa85"],[2120,1520,280,130,"#b4ac80"],[500,500,400,270,"#a4aa72"],[1840,825,180,110,"#ba977a"]]){c.save();c.translate(x,y);c.scale(1,ry/rx);const g=c.createRadialGradient(0,0,20,0,0,rx);g.addColorStop(0,tint+'95');g.addColorStop(1,tint+'00');c.fillStyle=g;c.fillRect(-rx,-rx,rx*2,rx*2);c.restore();}
