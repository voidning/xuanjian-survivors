...
function oldPaving(c){
 c.save();
 for(let row=0;row<13;row++){const yy=640+row*54,shift=row%2?43:0;
  for(let col=0;col<17;col++){const xx=900+col*86+shift,cx=xx+42,cy=yy+26;
   const edge=Math.max(0,1-Math.pow((cx-1570)/670,4)-Math.pow((cy-1030)/420,4));
   const bank=Math.max(0,Math.min(1,(creekY(cx)-cy-27)/95));
   const grove=Math.min(1,Math.hypot((cx-1110)/160,(cy-1020)/100),Math.hypot((cx-2010)/150,(cy-1020)/95));
   const alpha=edge*bank*grove*.42;if(alpha<.035)continue;
   const n=(row*17+col*31)%13,dx=n%3,dy=n%4;
   c.globalAlpha=alpha;
   shape(c,[[xx+3,yy+dy],[xx+81,yy+1],[xx+83,yy+48],[xx+5+dx,yy+50]],n%4===0?'#d3cebd':n%4===1?'#e2dbca':'#ece6d7');
   // Broken joints avoid a continuous grid across the landscape.
   if(n%5!==0)stroke(c,[[xx+7,yy+50],[xx+52+n,yy+49]],'#b7b3a2',.8);
   if(n%4!==0)stroke(c,[[xx+83,yy+5],[xx+82,yy+34+n]],'#b8b5a7',.75);
   if(n===2||n===9){stroke(c,[[xx+6,yy+2],[xx+14,yy+8],[xx+11,yy+14]],'#b7b1a0',.7);}
  }
 }
 c.restore();
}
