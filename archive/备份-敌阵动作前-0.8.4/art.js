...
if(kind===2){line([[-7,-29],[-14,-18],[-9,-13]],ink,3);line([[5,-29],[14,-20],[10,-13]],ink,3)}else{line([[0,-31],[-10,-22],[-16,-26]],ink,3);line([[0,-31],[10,casting?-36:-22],[casting?23:18,casting?-39:-27]],ink,3);const q=pose.attack||0,u=1-q,prep=pose.prepare||0,stroke=pose.empowered?'#b29a46':pose.spring?'#6faaa4':'#596268';c.save();c.translate(14,-28);const aim=pose.aim??-.7;c.rotate(Math.atan2(Math.sin(aim),Math.cos(aim)*dir));
if(kind===3){const reach=q>0?24*Math.pow(q,2)-5*Math.sin(u*Math.PI): -9*prep;line([[-26+reach,0],[38+reach,0]],stroke,2);line([[38+reach,0],[28+reach,-4],[31+reach,4],[38+reach,0]],stroke,1.7);}
else if(kind===4){const draw=q>0?Math.sin(u*24)*q*3:prep*11;const flex=q>0?Math.sin(u*20)*q*2:prep*3;line([[5,-19],[13+flex,-9],[16+flex,0],[13+flex,9],[5,19]],stroke,1.8);line([[5,-19],[-draw,0],[5,19]],stroke,1);if(!q||u>.65)line([[-draw-4,0],[24,0]],stroke,1.3);}
else{c.rotate(q?-.9+1.9*(1-Math.pow(q,3)):-.55-prep*.65);line([[-5,0],[31,0],[37,0]],stroke,2);line([[2,-5],[2,5]],ink,2);if(q){line([[9,-5],[27,-5],[35,-2]],stroke,1,q*.45);line([[10,-9],[24,-9],[34,-5]],stroke,.7,q*.22);}}
