...
const W=3200,H=2200,P={paper:'#e5e9da',earth:'#d1d4b9',grass:'#ccd8bc',water:'#b0cfc5',deep:'#86b7ad',edge:'#729e90',rock:'#bcc8b2',ink:'#789280',moss:'#93ad83'};
...
function rock(c,x,y,s=1,flat=false){c.save();c.translate(x,y);c.scale(s,s);ellipse(c,3,5,29,8,'#dce1d4');const top=flat?-13:-34-random()*17;
 shape(c,[[-28,0],[-20,top*.6],[-5,top],[17,top*.72],[30,-2],[14,7],[-16,6]],'#c6d0bc',P.ink);
 shape(c,[[-5,top],[17,top*.72],[30,-2],[7,3],[3,top*.5]],'#9bae9b');
 stroke(c,[[-20,top*.6],[-8,top*.55],[3,top*.5],[7,3]],'#7f9988');
...
function pine(c,x,y,s){c.save();c.translate(x,y);c.scale(s,s);ellipse(c,14,3,50,12,'#e0e5d9');
 shape(c,[[-5,0],[-3,-38],[8,-74],[3,-100],[11,-102],[16,-72],[4,-36],[3,0]],'#9cab8b','#677f68');
 const branches=[[-2,-37,-42,-65],[10,-64,49,-82],[7,-94,-19,-111],[9,-99,27,-122]];
 for(const [a,b,x2,y2] of branches){stroke(c,[[a,b],[(a+x2)/2,y2+9],[x2,y2]],'#677f68',2);for(let k=0;k<4;k++){const xx=x2+(k-1.5)*13,yy=y2+Math.sin(k*2)*5;c.beginPath();c.moveTo(xx-20,yy+3);c.bezierCurveTo(xx-24,yy-5,xx-15,yy-9,xx-9,yy-7);c.bezierCurveTo(xx-9,yy-16,xx+6,yy-16,xx+10,yy-8);c.bezierCurveTo(xx+21,yy-10,xx+29,yy+1,xx+18,yy+5);c.bezierCurveTo(xx+2,yy+9,xx-7,yy+4,xx-20,yy+3);c.fillStyle=k%2?'#8cac8b':'#a9c2a0';c.fill();c.strokeStyle='#769775';c.lineWidth=.65;c.stroke();for(let n=0;n<5;n++)stroke(c,[[xx-12+n*6,yy+1],[xx-8+n*6,yy-6]],'#6c8d70',.75);}}
...
function pavilion(c,x,y){c.save();c.translate(x,y);c.globalAlpha=1;
 ellipse(c,8,13,115,24,'#d5ddcc');
 for(let j=2;j>=0;j--)shape(c,[[-83-j*8,j*5],[83+j*8,j*5],[92+j*8,j*5+7],[-92-j*8,j*5+7]],j%2?'#d1d9c8':'#e0e5d6','#b5c1ac');
 for(const a of [-62,62]){shape(c,[[a-4,0],[a-4,-82],[a+4,-82],[a+4,0]],'#849778','#596f57');stroke(c,[[a-2,-77],[a-2,-6]],'#d8e1ca',1);}
 stroke(c,[[-62,-70],[62,-70]],'#8fa58a',5);stroke(c,[[-62,-21],[62,-21]],'#b5c3aa',2);
 for(let i=-45;i<=45;i+=15)stroke(c,[[i,-20],[i,-4]],'#b2c0a6',1.5);
 shape(c,[[-106,-68],[-83,-76],[-38,-97],[0,-107],[38,-97],[83,-76],[106,-68],[67,-69],[0,-85],[-67,-69]],'#668f80','#436d60');
 shape(c,[[-99,-68],[-65,-66],[0,-81],[65,-66],[99,-68],[61,-60],[0,-72],[-61,-60]],'#93b19a','#688c73');
 for(let i=-5;i<=5;i++){const u=i/5;stroke(c,[[u*34,-97+Math.abs(u)*6],[u*81,-72+Math.abs(u)*3]],'#dce6ce',1);}
 stroke(c,[[-32,-99],[0,-109],[32,-99]],'#879f80',2);shape(c,[[-18,-70],[18,-70],[18,-57],[-18,-57]],'#789676','#a9bb97');
 stroke(c,[[-10,-64],[-3,-64],[3,-64],[10,-64]],'#d8d7b4',1.5);
 grass(c,-105,17,1.1);grass(c,100,18,.9);c.restore();}
function bambooGrove(c,x,y){c.save();c.translate(x,y);for(let i=0;i<9;i++){const xx=i*13-54,hh=58+(i%4)*14;stroke(c,[[xx,0],[xx+4,-hh]],'#7c9d79',2.1);for(let j=1;j<4;j++){const yy=-hh*j/4;stroke(c,[[xx-2,yy],[xx+5,yy]],'#dce4ce',1);const side=(i+j)%2?1:-1;stroke(c,[[xx+2,yy],[xx+side*19,yy-12]],'#719775',.8);shape(c,[[xx+side*11,yy-7],[xx+side*33,yy-19],[xx+side*23,yy-6]],'#90af87');}}c.restore();}
...
function terrain(c){
 ...
 c.lineCap='round';path();c.strokeStyle='#c5cbb0';c.lineWidth=76;c.stroke();path();c.strokeStyle='#d8dbc2';c.lineWidth=56;c.stroke();
 ...
 pavilion(c,1110,830);
 for(const [x,y,sc] of [[1160,1030,.9],[1210,1060,.66],[2030,910,1.2],[2100,953,.85],[1960,1460,.8]])pine(c,x,y,sc);
 for(const [x,y,sc] of [[1200,1085,.65],[2020,982,.8],[2080,1005,.5],[1860,1470,.75]])rock(c,x,y,sc);
bambooGrove(c,970,910);bambooGrove(c,1270,755);ruin(c,1670,430);ruin(c,2480,1700);
 ...
 for(let i=0;i<75;i++){const x=random()*W,y=creekY(x)+(i%2?153:-14);for(let j=0;j<3;j++){const dx=j*5;stroke(c,[[x+dx,y],[x+dx-3,y-17],[x+dx+1,y-30-j*3]],'#789a79',.8);ellipse(c,x+dx+1,y-31-j*3,1.4,3,'#c2c7ad');}}
 for(let i=0;i<46;i++){const x=120+random()*(W-240),y=140+random()*(H-280);if(y<creekY(x)-55||y>creekY(x)+185)botanical(c,x,y);}
 c.save();c.globalAlpha=.075;for(let i=0;i<24000;i++){const x=random()*W,y=random()*H;c.fillStyle=i%2?'#fffbea':'#566d52';c.fillRect(x,y,1+random()*2,1);}c.restore();
