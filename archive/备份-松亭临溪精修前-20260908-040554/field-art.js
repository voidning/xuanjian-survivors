...
function shadow(c,x,y,rx,ry){ellipse(c,x+8,y+4,rx,ry,'#555f5912');ellipse(c,x+3,y+2,rx*.7,ry*.65,'#555f5918');}
function rock(c,variant=0){const h=variant?55:78;
 shadow(c,0,0,51,13);
 shape(c,[[-43,0],[-34,-h*.48],[-10,-h],[6,-h+3],[38,-h*.5],[49,-4],[22,10],[-20,7]],'#d6d7cf','#74786f',1.6);
 shape(c,[[-10,-h],[7,-h+3],[38,-h*.5],[49,-4],[19,5],[9,-h*.43]],'#9ea9a8');
 ...
 for(let i=0;i<10;i++){const x=-22+i*5;stroke(c,[[x,-5],[x+4,-12-(i%3)*3]],i%2?'#929688':'#e8e4d5',.9);}
...
function pine(c,variant=0){shadow(c,8,3,75,17);
 ...
 c.save();c.translate(cx+(variant?5:0),cy);c.scale(rx/45,1);
 ...
 for(let j=0;j<30;j++){const x=-40+(j%10)*8+Math.sin(j*3)*2,y=-12+Math.floor(j/10)*7;stroke(c,[[x-3,y],[x,y-5],[x+3,y]],j%3?'#bcc2a0':'#4b6242',.65);}
 c.restore();}
...
function water(c){
 ...
 for(let i=0;i<140;i++){const x=random()*W,y=creekY(x)+(i%2?146:-14);ellipse(c,x,y,2+random()*4,1.2,'#a5ac8b');}
}
function court(c){c.save();c.translate(1600,1090);ellipse(c,2,6,165,97,'#656e6125');ellipse(c,0,3,161,94,'#b1b1a2');ellipse(c,0,-2,157,91,'#e6e0d1');
 for(let ring=0;ring<3;ring++){const rx=94+ring*24,ry=53+ring*14;for(let j=0;j<12;j++){const a=j*Math.PI/6;c.beginPath();c.ellipse(0,-2,rx,ry,0,a+.025,a+Math.PI/6-.035);c.strokeStyle=ring===2?'#9b9c8d':'#c9c4b5';c.lineWidth=ring===2?1.5:1;c.stroke();if(ring<2)stroke(c,[[Math.cos(a)*rx,Math.sin(a)*ry-2],[Math.cos(a)*(rx+24),Math.sin(a)*(ry+14)-2]],'#bdb9a8',.8);}}
...
function build(){cacheBuilds++;seed=73051;painting=document.createElement('canvas');painting.width=W;painting.height=H;const c=painting.getContext('2d');c.fillStyle=P.paper;c.fillRect(0,0,W,H);
 ...
 path(c,'M1670 270C1760 620 1410 820 1560 1160C1660 1460 1850 1740 2460 2160',null,'#d2c8b4',74);path(c,'M1670 270C1760 620 1410 820 1560 1160C1660 1460 1850 1740 2460 2160',null,'#e4daca',61);court(c);water(c);
 // Deliberate scenic islands frame the starting court, with a clear approach on each side.
 for(const [x,y,s]of [[1090,1060,1],[2020,1030,1.1],[2140,1510,.9],[480,500,1.1],[2550,530,1.2],[550,1820,1],[2630,1850,1]]){
 place('rock',x-48,y+7,s*.72);place('pine',x,y,s);place('rock',x+39,y+27,s*.5,1);place('bamboo',x+106,y-46,s*.68);
 }
 place('pavilion',1140,845,1.18);place('bamboo',973,878,.95);place('bamboo',1290,760,.85);place('rock',1045,918,.52,1);
 ...
 c.font='12px Songti SC,serif';c.textAlign='center';c.fillStyle='#858272';c.fillText('松亭',1140,885);c.fillText('残碑',1840,859);c.fillText('溪岸',1650,creekY(1650)+171);
 // Bank reeds use short groups, leaving crossing stones exposed.
 for(let i=0;i<28;i++){const x=100+i*111,y=creekY(x)+(i%2?145:-15);for(let j=0;j<5;j++){const xx=x+j*4,h=11+(j%3)*5;stroke(c,[[xx,y],[xx-2,y-h]],i%4===0?'#a89a72':'#899773',.8);stroke(c,[[xx-2,y-h*.6],[xx+3,y-h]],i%4===0?'#c1b085':'#aab18c',.8);}}
 // Restrained leaf litter belongs to the stele and pavilion, not the open court.
 for(const [cx,cy]of [[1815,841],[1150,870],[2460,1710]])for(let j=0;j<9;j++){const x=cx+Math.sin(j*2.4)*28,y=cy+Math.cos(j*1.7)*10;shape(c,[[x-3,y],[x,y-2],[x+4,y+1],[x,y+2]],j%3?'#b5a185':'#c5b384');}
