...
(function(root){'use strict';const X=typeof module!=='undefined'?require('./engine.js'):root.XJ;const {Run,dist,clamp,W,H}=X;
const turn=(a,b,step)=>a+clamp(Math.atan2(Math.sin(b-a),Math.cos(b-a)),-step,step);
Run.prototype.firePower=function(){return (this.gift==='firegift'?1.2:1)*(this.item==='lamp'?1.15:1);};
Run.prototype.updateSkills=function(dt,nearest){const p=this.p;
if(this.lv('body')){let count=0,weight=0;this.enemies.forEach(e=>{if(e.born<=0&&dist(e,p)<170){count++;weight+=e.boss?3:e.elite?2:1;}});p.pressure=count>=2?Math.min(1,weight/5):0;p.danger=p.pressure;
...
const order=['gate','edict','dusk','light','rain','spring','muddle','conceal','dew','thunder','armor','flame','angler','peril'];const shift=Math.floor(this.t*.5)%order.length;
for(let j=0;j<order.length;j++){const id=order[(j+shift)%order.length],lv=this.lv(id);if(!lv||this.cool[id]>0)continue;if(!nearest&&!['dew','angler','peril'].includes(id))continue;
if(['angler','peril'].includes(id)&&!this.nearest(p,430))continue;
if(id==='spring'&&this.cool.springPower>0&&this.springCharges>0)continue;
if(id==='conceal'&&!this.nearest(p,230))continue;
if(id==='dew'&&!(p.slow>0)&&!this.zones.list().some(z=>z.kind==='hazard'&&dist(z,p)<z.r+60))continue;
if(id==='gate'&&this.allies.count>=Math.min(8,lv+1+(this.fruit?2:0))){this.cool.gate=.5;continue;}
if(id==='thunder'&&!this.nearest(p,175))continue;
if(id==='grove'&&p.hp>=this.maxHP-4)continue;
if(id==='light'&&!this.nearest(p,195+lv*25)&&!this.allies.count)continue;
if(id==='flame'&&!this.nearest(p,155+lv*20))continue;
if(id==='armor'&&p.armor>12&&p.armorTime>1)continue;
if(id==='pure'&&p.guard>0&&!this.zones.list().some(z=>z.kind==='hazard'&&dist(z,p)<120)&&!this.bullets.list().some(b=>b.enemy&&b.kind!=='metal'&&dist(b,p)<180))continue;
if(['edict','dusk','pure','mountain','flame','rain','grove','angler','peril'].includes(id)&&this.zones.count>=this.zones.max)continue;
const cost={angler:26,peril:28,spring:22,muddle:20,conceal:18,dew:22,gate:26,edict:14,dusk:20,light:26,pure:18,mountain:24,thunder:28,armor:20,flame:18,rain:22,grove:24}[id];if(!this.spend(cost))continue;p.cast=.4;this.skillSignal?.(id);
if(id==='angler'){
...
if(id==='peril'){
 ...
 this.zones.add({kind:'peril',x:target.x,y:target.y,r,life:2.6+lv*.6,age:0,tick:0,lv});
...
if(id==='spring'){this.springCharges=this.fruits.lushui?5:4;this.cool.springPower=8;this.cool.spring=7;this.effect('spring',p.x,p.y,65,.6);}
...
if(id==='rain'){nearest=this.cluster(500,110+lv*20)||nearest;this.zones.add({kind:'rain',x:nearest.x,y:nearest.y,r:110+lv*20,life:4+lv*.5,age:0,tick:0,lv});this.cool.rain=9;}
...
Run.prototype.updateBullets=function(dt){this.intercept();baseBullets.call(this,dt);};
...
if(z.kind==='peril'){this.enemies.forEach(e=>{if(e.born<=0&&dist(e,z)<z.r)e.slow=Math.max(e.slow,.2);});}
...
if(z.kind==='rain'&&this.fruits.lushui){z.x+=(this.p.x-z.x)*Math.min(1,dt*2);z.y+=(this.p.y-z.y)*Math.min(1,dt*2);}
