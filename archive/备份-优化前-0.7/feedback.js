...
const verbs={angler:'幻饵落地 · 诱敌聚群',peril:'山川收束 · 陷敌迟行',gate:'甲兵入场 · 追击近敌',edict:'展开令域 · 普攻追加打击',dusk:'血漠展开 · 记伤与挪移',light:'六合禁闭 · 消弹、振兵',spring:'泉势加持 · 强化普攻',muddle:'浊光惑敌 · 扰乱追击',conceal:'藏匿身形 · 留下残息',dew:'洗去法区 · 护行生效',thunder:'雷霆荡清 · 近敌震退',armor:'金胄凝成 · 吸收伤害',flame:'吐焰炎行 · 持续灼伤',rain:'青雨落阵 · 磨伤破护'};
...
return c[id]>0?'冷却 '+c[id].toFixed(1)+'s':'待机 · 等待敌情或法力';}
...
out+=(run.skillSignals||[]).filter(e=>run.t-e.at<3.2).map(e=>`<div class="skill-event"><b>${names[e.id]||e.id}</b><span>${e.text}</span></div>`).join('');return out;}
