...
const costs=Object.freeze({dali:26,fragrance:24,angler:26,peril:28,spring:22,muddle:20,conceal:18,dew:22,gate:26,edict:14,dusk:20,light:26,pure:18,mountain:24,thunder:28,armor:20,flame:18,rain:22,grove:24});
...
Run.prototype.skillBlock=function(id,nearest){const p=this.p;if(!this.lv(id))return '尚未修习';if(this.cool[id]>0)return '冷却 '+this.cool[id].toFixed(1)+'s';
 ...
 if(['dali','fragrance','edict','dusk','rain','flame','angler','peril'].includes(id)){if(this.zones.count>=this.zones.max)return '法域已满';if(this.zones.list().some(z=>z.kind===id&&z.life-z.age>.25))return '法域仍有效';}
 return '';
};
