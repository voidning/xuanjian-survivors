...
for(const dao of (rainAware?['lushui']:['mingyang','lushui']))for(const item of ITEMS.filter(x=>x.id!=='screen'))for(const seed of [42,907]){
...
console.log(JSON.stringify({rainAware,fourOnly,method:'Input-only deterministic simulation, Configured routes × 4 weapons × 2 seeds. Natural health, experience, offers, enemy timing. No injected progression. Not browser or human E2E.',root,runs},null,2));
