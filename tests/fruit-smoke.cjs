...
for(const route of RECIPES)for(const item of ITEMS)for(const seed of [42,907])for(const strategy of ['route','balanced']){
 ...
 while(r.t<limit&&r.state!=='ended'){
  if(r.state==='choice'){
   ...
   const selected=kind==='gift'?'whale':eligible.slice().sort((a,b)=>rank(r,b,targets)-rank(r,a,targets))[0];
   ...
   if(r.state==='running'){const primary=route.id==='mingyang'?'gate':'rain';if(r.canPrimary(primary))r.setPrimary(primary);}
   continue;
  }
  if(r.t>=nextMove){input=steer(r);nextMove=r.t+.25;}if(r.hasFruitActive?.()&&r.fruitActiveEnergy()>=100&&r.enemies.list().filter(e=>e.born<=0&&dist(e,r.p)<300).length>=3)r.castFruitActive();r.step(.05,input);
 }
 const p=progress(r,route);runs.push({route:route.id,item:item.id,seed,strategy,time:+r.t.toFixed(2),reason:r.result?.reason||'time limit',hp:+r.p.hp.toFixed(2),won:r.result?.reason==='破阵功成',formedAt,fiveAt,activeUses:r.telemetry.fruitActives||0,trainingChoices,routeOpportunitySkippedForTraining:skipped,longestFreshUpgradeWithoutProgressOpportunity:longest,rerollsUsed:rerolls,rerollsLeft:r.rerolls,upgrades,level:r.level,kills:r.kills,events:r.fieldHistory||[],eventChoices:r.fieldRewards||0,primary:r.primarySkill||null,missing:p.missing,needsMastery:!p.mastered,skills:r.skills,training:r.trainingSnapshot(),fruits:r.fruits,lastDamage:r.result?.lastDamage||[],choices});
 process.stderr.write(`${runs.length}/40 ${route.id} ${item.id} ${seed} ${strategy}: ${r.t.toFixed(1)}s fruit=${formedAt} ${r.result?.reason||'limit'}\n`);
}
console.log(JSON.stringify({root,method:'Input-only deterministic simulation, not browser or human E2E. Natural opening/offers/XP/HP/time. Same avoidance policy as encounters-smoke. Gift whale; primary gate/rain when owned; max 2 rerolls per decision with no target, except every third ordinary upgrade in balanced chooses training if available. Route targets prefer missing skills, then highest rank toward mastery only where fruitProgress exists. Initial-offer drought counts ordinary upgrade screens before rerolls while targets remain. Deaths retained.',limit,runs},null,2));
