...
for(const dao of (rainAware?['lushui']:['mingyang','lushui']))for(const item of ITEMS.filter(x=>x.id==='dasheng'))for(const seed of [42,907]){
 ...
 while(r.t<limit&&r.state!=='ended'){
  if(r.state==='choice'){
   ...
   const rank=id=>{if(fourOnly&&r.masteredCount()>=4&&r.lv(id)===2)return -1000;if(r.choiceKind==='opening')return id===(dao==='mingyang'?'gate':'rain')?1000:0;if(r.choiceKind==='gift')return id==='whale'?100:0;
    ...
    return (r.lv(id)?45:0)+({rain:26,gate:25,light:24,gold:23,thunder:20,flame:18,armor:16,body:14,dew:13,spring:12}[id]||10);
   };
...
assert.equal(runs.length,rainAware?2:4);assert(runs.every(r=>r.maxSkills<=5&&Object.keys(r.fruits).length===0));
console.log(JSON.stringify({rainAware,fourOnly,method:'Input-only deterministic simulation, Configured routes × 1 weapon × 2 seeds. Natural health, experience, offers, enemy timing. No injected progression. Not browser or human E2E.',root,runs},null,2));
