...
const W=3200,H=2200,P={paper:'#f2efe5',line:'#797c70',light:'#e3dfd1',water:'#aecbd4',deep:'#92b5c1',ink:'#304e48',gold:'#aa9a6a'};
...
function pine(c,variant=0){shadow(c,8,3,75,17);
 path(c,'M-8 1Q-10-40 5-83Q17-115 9-149L17-151Q29-115 15-79Q2-40 1 0Z','#937052','#594c3c',1.5);
 ...
 for(const [x,y,tx,ty]of branches){path(c,`M${x} ${y}Q${(x+tx)/2} ${ty+4} ${tx} ${ty}`,null,'#594c3c',5);path(c,`M${x} ${y-2}Q${(x+tx)/2} ${ty+1} ${tx} ${ty-2}`,null,'#bda17a',1.1);}
 ...
 path(c,'M-47 3Q-54-5-40-13Q-33-24-18-21Q-8-35 6-23Q22-29 29-18Q50-19 51-5Q56 4 39 9Q10 17-16 10Q-35 14-47 3Z','#55734c','#455a40',1.2);
 path(c,'M-42-7Q-27-25-15-17Q0-31 12-18Q27-25 37-10Q11-3-8-8Q-26-2-42-7Z','#91a16b');
 // Irregular needle fans follow branches, avoiding a tiled chevron texture.
 path(c,'M-43 2Q-17-6 0 1Q24-9 45-2L37 8Q12 14-10 7Q-30 12-43 2Z','#344f3a');
 ...
 path(c,'M-39 5Q-20 1-7 5M7 4Q22-2 36 2',null,'#b9ad78',.75);
 c.restore();}
...
function bamboo(c){shadow(c,2,2,60,12);
 // Root clusters sit on the soil; the leaf detail stays in the existing grove.
 for(const [x,y]of [[-36,2],[-13,5],[20,3],[43,0]]){ellipse(c,x,y,13,3,'#7c89634a');for(let j=0;j<3;j++)path(c,`M${x+j*3} ${y}q-4-6 ${-5+j*3}-11`,null,'#687a48',.8);}

 for(let i=0;i<7;i++){const x=(i-3)*15,h=94+(i%3)*20,lean=(i-3)*2;stroke(c,[[x,0],[x+lean,-h]],'#938958',3);stroke(c,[[x-1,-2],[x+lean-1,-h]],'#c8bd81',1);
 ...
 for(let k=0;k<3;k++){const a=xx+sign*(9+k*7),b=y-7-k*4;path(c,`M${a} ${b}q${sign*5} -18 ${sign*16} -20q${-sign*1} 15 ${-sign*16} 20Z`,k%2?'#426a4b':'#9dac69');}
 }}
}
function pavilion(c){shadow(c,0,6,145,24);
 for(let j=3;j>=0;j--){const left=-91-j*8,right=91+j*8,yy=j*5;shape(c,[[left,yy],[right,yy],[right+17,yy+9],[left-17,yy+9]],'#d9d5c6','#8e9184');shape(c,[[left-17,yy+9],[right+17,yy+9],[right+17,yy+12],[left-17,yy+12]],'#8b8e82');stroke(c,[[left-15,yy+8],[right+15,yy+8]],'#f0e9d5',1);for(let k=-2;k<=2;k++)stroke(c,[[k*37+j%2*15,yy+1],[k*37+j%2*15+8,yy+8]],'#bab6a5',.65);}

 shape(c,[[-67,-89],[67,-89],[67,-5],[-67,-5]],'#74513423');shape(c,[[-78,-2],[69,-2],[94,7],[-93,7]],'#89816a42');
 // Rear timber stays lighter; eave shadow makes the open pavilion legible.
 for(const x of [-43,43]){shape(c,[[x-3,-12],[x-3,-105],[x+3,-105],[x+3,-12]],'#b19470','#81735e',.8);stroke(c,[[x-2,-99],[x-2,-14]],'#c7b796',1);}
 shape(c,[[-70,-86],[70,-86],[67,-69],[-67,-69]],'#453d354a');
 for(const x of [-73,73]){shape(c,[[x-5,0],[x-5,-101],[x+5,-101],[x+5,0]],'#916447','#604833');stroke(c,[[x-3,-95],[x-3,-5]],'#c4b493',1.3);shape(c,[[x-9,2],[x-8,-7],[x+8,-7],[x+9,2]],'#c3c2b1','#898e81');}
 ...
 stroke(c,[[-75,-90],[75,-90]],'#624430',7);stroke(c,[[-72,-25],[72,-25]],'#a38159',4);
 for(let i=-60;i<=60;i+=20){stroke(c,[[i,-23],[i,-2]],'#9f805d',2);stroke(c,[[i,-20],[i+15,-4],[i+15,-20],[i,-4]],'#baa88a',1);}
 ...
 // Distinct roof facets: pale upper-left slope, cool shaded right slope.
 path(c,'M0-159Q40-145 70-122Q99-97 132-94L105-86Q49-101 0-124Z','#3e595d',null);
 path(c,'M-122-96Q-76-109-4-153L-4-130Q-57-101-103-88Z','#7b9390',null);
 ...
 // Compact bracket silhouettes, all cached with the pavilion sprite.
 for(const x of [-73,73]){shape(c,[[x-15,-90],[x+15,-90],[x+11,-84],[x+6,-84],[x+6,-78],[x-6,-78],[x-6,-84],[x-11,-84]],'#5f4e3d','#b3a080',.7);stroke(c,[[x-11,-88],[x+11,-88]],'#d0ba90',1);}
...
function stele(c){shadow(c,0,0,38,10);shape(c,[[-25,1],[25,1],[32,9],[-32,9]],'#b7a68e','#898477');shape(c,[[-17,0],[-17,-76],[-9,-91],[13,-88],[20,-77],[20,0]],'#d4c7ae','#7b6c59',1.4);shape(c,[[13,-88],[20,-77],[20,0],[12,0]],'#9c9484');stroke(c,[[-8,-78],[-8,-14],[7,-14]],'#a7a394');stroke(c,[[-9,-91],[-4,-66],[-7,-47],[2,-34]],'#8a8476',1);for(let j=0;j<5;j++){const y=-66+j*9;stroke(c,[[-2,y],[6,y],[6,y+3],[-1,y+3]],'#888579',1.2);}}
...
function water(c){
 ...
 ribbon(-23,167,'#dce0d4');ribbon(-8,146,'#9fb6b6');ribbon(0,127,P.water);ribbon(30,52,'#87afc1');ribbon(114,12,'#a3c1bd');
...
// Faint remnants of courtyard paving; cached once, with no collision meaning.
function oldPaving(c){
 // Whole-world fine grid: no alternating fills, bright grout or colour halos.
 c.save();
 // Two world-anchored slopes form 128 by 72 diamonds, independent of movement.
 c.globalAlpha=.12;
 for(let b=-W*.5625-72;b<=H+W*.5625+72;b+=72){
  stroke(c,[[0,b],[W,b+W*.5625]],'#999485',.85);
  stroke(c,[[0,b],[W,b-W*.5625]],'#999485',.85);
 }

let state=48127;const rnd=()=>{state=(Math.imul(state,1664525)+1013904223)>>>0;return state/4294967296;};
 for(let i=0;i<420;i++){const x=rnd()*W,y=rnd()*H,len=3+rnd()*10;
  c.globalAlpha=.07+rnd()*.055;
  stroke(c,[[x,y],[x+len*.45,y-.5],[x+len,y+.3]],'#8c887a',.65);
 }
 // Only the terrace approach retains fragments of old paving; open edges merge into soil.
 for(let i=0;i<24;i++){const x=1180+rnd()*320,y=1008+rnd()*115;
  c.globalAlpha=.13;const len=15+rnd()*25;
  stroke(c,[[x,y],[x+len,y+.7]],'#aba594',.7);
  if(i%4===0)stroke(c,[[x+len,y+.7],[x+len+1,y+9]],'#aba594',.6);
 }
 c.restore();
}
function build(){cacheBuilds++;seed=73051;painting=document.createElement('canvas');painting.width=W;painting.height=H;const c=painting.getContext('2d');c.fillStyle=P.paper;c.fillRect(0,0,W,H);
 oldPaving(c);
 // Ground stays neutral; colour belongs to material, not radial washes.
 ...
 for(const [cx,side]of [[365,1],[1095,-1],[1190,-1],[1920,-1],[2240,1],[2730,-1]])for(let i=0;i<4;i++){const x=cx+i*8,y=creekY(x)+(side>0?148:-13);for(let j=0;j<3;j++){const xx=x+j*3,h=13+(j+i)%3*6;path(c,`M${xx} ${y}q2-${h*.5} ${4+j}-${h}`,null,'#9c926a',.8);ellipse(c,xx+4+j,y-h,1.3,4,'#b79a59');stroke(c,[[xx,y-4],[xx-4,y-11]],'#90956f',.7);}}
 ...
 for(const [cx,cy]of [[1815,841],[1150,1040],[2460,1710]])for(let j=0;j<9;j++){const x=cx+Math.sin(j*2.4)*28,y=cy+Math.cos(j*1.7)*10;shape(c,[[x-3,y],[x,y-2],[x+4,y+1],[x,y+2]],j%3?'#ac8b62':'#c8af72');}
...
function ground(c,t,cam,w,h,zoom,line,oval,reduced){if(!painting)build();const margin=3,left=Math.max(0,Math.floor(cam.x-w/zoom/2)-margin),top=Math.max(0,Math.floor(cam.y-h/zoom/2)-margin),right=Math.min(W,Math.ceil(cam.x+w/zoom/2)+margin),bottom=Math.min(H,Math.ceil(cam.y+h/zoom/2)+margin);if(right>left&&bottom>top)c.drawImage(painting,left,top,right-left,bottom-top,left,top,right-left,bottom-top);if(!reduced){c.save();c.globalAlpha=.65;for(let i=0;i<18;i++){const x=80+i*173,y=creekY(x)+48+i%3*19;if(Math.abs(x-cam.x)>w/zoom/2+40||Math.abs(y-cam.y)>h/zoom/2+30)continue;const d=Math.sin(t*.45+i)*4;stroke(c,[[x-10+d,y],[x+12+d,y]],'#f2f8f0',1);}c.restore();}}
const composeQueue=[];
function compose(c,cam,w,h,zoom,actors){if(!painting)build();composeQueue.length=0;for(const a of actors)composeQueue.push(a);const rx=w/zoom/2,ry=h/zoom/2;for(const o of props)if(Math.abs(o.x-cam.x)<rx+o.w*o.scale&&Math.abs(o.y-cam.y)<ry+o.h*o.scale)composeQueue.push({y:o.y,prop:o});composeQueue.sort((a,b)=>a.y-b.y);for(const entry of composeQueue){if(!entry.prop){entry.draw();continue;}const o=entry.prop,left=o.x-o.w*o.scale/2,top=o.y-(o.h-30)*o.scale;let occludes=false;for(const a of actors)if(a.x>left+15&&a.x<left+o.w*o.scale-15&&a.y<o.y+3&&a.y>top+22){occludes=true;break;}c.save();if(occludes)c.globalAlpha*=.28;c.drawImage(o.canvas,left,top,o.w*o.scale,o.h*o.scale);c.restore();}}
function overlay(c,run,cam,w,h,zoom,line,oval,label,reduced){
 ...
 const rx=Math.max(40,w/2-65),ry=Math.max(40,h/2-165),strong=[];run.enemies.forEach(e=>{if(!(e.elite||e.boss)||e.born>0)return;const dx=(e.x-cam.x)*zoom,dy=(e.y-cam.y)*zoom;if(Math.abs(dx)<rx&&Math.abs(dy)<ry)return;const score=(e.boss?1e12:0)-((e.x-run.p.x)**2+(e.y-run.p.y)**2);if(strong.length<2){strong.push({e,score});strong.sort((a,b)=>b.score-a.score);}else if(score>strong[1].score){strong[1]={e,score};strong.sort((a,b)=>b.score-a.score);}});
 for(const item of strong){const e=item.e,dx=(e.x-cam.x)*zoom,dy=(e.y-cam.y)*zoom,k=Math.min(rx/(Math.abs(dx)||1),ry/(Math.abs(dy)||1)),x=w/2+dx*k,y=h/2+dy*k,a=Math.atan2(dy,dx);c.save();c.translate(x,y);c.rotate(a);line([[-7,-5],[0,0],[-7,5]],e.boss?'#9b5f50':'#927f56',1.8,.85);c.restore();label(e.boss?'首领':'精英',x,y+18,e.boss?'#9b5f50':'#857859',10);}
