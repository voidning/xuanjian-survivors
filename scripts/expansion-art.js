...
function zone(c,run,z,line,oval,label,reduced=false){
 const motionAge=reduced?0:z.age;
 ...
 if(z.kind==='angler'){
  ...
  const bob=Math.sin(motionAge*4)*3,x=z.x,y=z.y-17+bob;
  line([[x,y-32],[x,y-9],[x+4,y-6],[x+8,y-10]],col,1.6);
  oval(x,y,7,7,'#f4f7f3');oval(x,y,7,7,col,true,1.8);
  for(let i=0;i<3;i++){const q=(motionAge*.45+i/3)%1;oval(z.x,z.y,14+q*37,5+q*13,col,true,1);}
  label('趋饵 · 受击清醒',z.x,z.y+z.r-16,col,10);
 }
 if(z.kind==='peril'){
  if(z.linked&&z.age<.65){const q=z.age/.65;for(let j=0;j<4;j++){const a=j*Math.PI/2+.6,r=z.r*(1-q*.7);line([[z.x+Math.cos(a)*r,z.y+Math.sin(a)*r],[z.x+Math.cos(a)*(r-15),z.y+Math.sin(a)*(r-15)]],'#668e92',1.5,1-q);}}
  ...
  for(let i=0;i<4;i++){
   const r=20+i*16,a=motionAge*.5+i*.7;
 ...
 if(z.kind==='flame'){
  ...
  for(let i=0;i<11;i++){
   const a=-z.spread+(i+.5)*z.spread*2/11,q=(motionAge*1.25+i*.137)%1;
...
function effect(c,run,f,line,oval,label,reduced=false){
 ...
 if(f.kind==='thunder'){
  const col='#756e9b';oval(f.x,f.y,f.r*(.8+q*.2),f.r*(.8+q*.2),col,true,1);
  for(let i=0;i<8;i++){
   const a=i*Math.PI/4,r=f.r*(.25+Math.min(1,q*3)*.75),ux=Math.cos(a),uy=Math.sin(a);
   ...
   line(points,col,2.6);line(points,'#e7edf1',.8,.9);if(i%2===0)line([points[1],[f.x+ux*r*.5-uy*31,f.y+uy*r*.5+ux*31]],col,1.1,.65);
  }
 }
 if(f.kind==='light'){
  const col='#a58b4a';for(let i=0;i<6;i++){
   const a=i*Math.PI/3,r=f.r*(.85+.15*Math.min(1,q*3)),x=f.x+Math.cos(a)*r,y=f.y+Math.sin(a)*r;
   line([[x,y+8],[x,y-32-Math.sin(q*Math.PI)*22]],col,2.1);line([[x-5,y-24],[x+5,y-24],[x,y-19],[x,y-11],[x+4,y-7]],col,1.1);
