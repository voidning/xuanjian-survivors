...
function clothing(c,e,ink){if(e.type===12)return;c.save();const heavy=e.type===5||e.boss,robe=[3,9,10].includes(e.type),fast=[0,7,11].includes(e.type),fill=heavy?'#7d756b':robe?'#777083':fast?'#89755f':'#a4977b',accent=e.elite||e.boss?'#a78c53':'#695d51';
 ...
 if([0,2,3,5].includes(e.type)){
 const shadow=e.type===3?'#494353':e.type===5?'#55595b':'#61503f';
 shape(c,[[3,-27],[8,-24],[7,-18],[10,-9],[5,-9],[0,-18]],shadow);
 if(e.type===0){shape(c,[[-7,-28],[-2,-29],[4,-19],[8,-18],[7,-15],[1,-16]],'#554636');shape(c,[[-7,-15],[-2,-15],[-4,-9],[-9,-8]],'#b2a083');}
 if(e.type===2){shape(c,[[-7,-27],[-2,-29],[5,-18],[2,-15]],'#6a5844');line(c,[[-6,-26],[2,-17]],'#c1ae86',1);}
 if(e.type===3){shape(c,[[-9,-24],[-5,-28],[-1,-20],[-6,-12],[-13,-5],[-15,-3]],'#91899c');line(c,[[-6,-26],[-1,-21],[-6,-11]],'#c1b8b1',1.1);}
 if(e.type===5){shape(c,[[-8,-29],[7,-29],[8,-22],[-2,-20],[-8,-23]],'#c0c2b9');shape(c,[[0,-28],[7,-28],[8,-22],[0,-20]],'#858e8e');line(c,[[-8,-29],[7,-29]],'#e0d9bf',1.4);for(const side of [-1,1])shape(c,[[side*4,-15],[side*11,-16],[side*13,-8],[side*5,-8]],'#717676','#454b4c',.8);}
 }
...
function finish(c,e,ink,t,reduced=false){if(e.type===12)return;c.save();
 ...
 // Compact jaw shadow gives depth without small facial ornament.
 if([0,2,3,5].includes(e.type)){shape(c,[[3,-43],[6,-41],[5,-34],[1,-30],[-3,-31],[2,-34]],'#b7ab98');line(c,[[2,-39],[5,-39]],'#454441',.9);}
