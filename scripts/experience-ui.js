...
const gifts={whale:['法力更充足，支持持续施法。','被动 · 上限 160，基础回复 13/秒'],life:['燃寿换法力，短时大幅增伤。','Q 手动 · 耗 1 寿元（不可恢复），回法 80；增伤 85% 持续 6 秒'],sparrow:['近身见血或自身受伤，激发悍勇。','被动 · 5秒悍勇，8秒触发间隔'],sunseal:['击败精英、首领，额外获得法力与经验。','被动 · 普通击杀不触发'],cloud:['驾雾轻身，移动速度提高。','被动 · 移动 +14%；不免伤、不穿墙'],might:['强化兵器普通攻击。','被动 · 仅强化剑、弓、枪、戟的实体攻击'],omen:['提示迫近危险的方向。','被动 · 需自行走位避险'],rainbow:['主动术法施放后短暂化解近身飞弹。','被动 · 有触发间隔与化解数量上限'],frostpine:['受创时保持身形稳定。','被动 · 减轻地面法区造成的受创迟滞'],greed:['强化南明心火并由心火击杀助修。','被动 · 需离火大离书三重方可受箓']};
function giftInfo(run,id){const d=X.GiftSystem?.details?.(run,id)||X.GiftSystem?.get?.(id);return d?[d.role||d.purpose||d.description||gifts[id]?.[0],d.desc||d.mechanic||gifts[id]?.[1],d]:[gifts[id]?.[0],gifts[id]?.[1],null];}
...
function growth(run,id,oldFruits){const training=trainingInfo(run,id);if(training)return training.title+' · '+training.stat(trainingRank(run,id))+(id==='vigor'?' · 气血已回复':'');if(foundationFuture[id])return name(id)+' · '+run.skillStage(id)+' · '+(run.lv(id)===3?foundationFuture[id]:run.lv(id)===1?foundationPlain[id]:run.foundationGain(id,2));if(gifts[id]||X.GiftSystem?.get?.(id)){const info=giftInfo(run,id),entry=info[2]||GIFTS.find(g=>g.id===id);return (entry?.name||id)+' · '+(entry?.role||info[1]);}return '';
}
