...
function draw(c,run,line,oval,label=()=>{}){c.save();
 ...
 run.enemies.forEach(e=>{if(e.duelId&&!e.final||e.phase!=='warn'||![1,6,7,11,12].includes(e.type))return;const length=e.type===7?158.4:e.type===12?190:e.type===11?(e.elite?360:300):312,half=e.type===11?30:e.r+14;c.setLineDash([7,5]);for(const sign of [-1,1]){const nx=-e.ay*half*sign,ny=e.ax*half*sign;line([[e.x+nx,e.y+ny],[e.x+e.ax*length+nx,e.y+e.ay*length+ny]],'#a05c40',2,.95);}c.setLineDash([]);});
 ...
 run.enemies.forEach(e=>root.XJDuelArt?.warning(c,e,line,oval,label));
