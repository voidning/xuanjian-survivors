...
for(const id of order){if(this.skillBlock(id,nearest))continue;const lv=this.lv(id),cost=this.skillCost(id);if(!this.spend(cost)){if(manual)break;this.skillBudget??=id;if(!waited){const log=this.loopLog();log.manaWait+=Math.max(0,Math.min(.05,dt));log.lastManaWaitAt=this.t;waited=true;}if(this.reserveSkill(id)){this.skillBudget=id;break;}continue;}delete this.skillWait[id];this.skillCasts??={};this.skillCasts[id]=(this.skillCasts[id]||0)+1;p.cast=.4;this.skillSignal?.(id);this.giftCast?.();
...
if(z.kind==='dali'){z.x=this.p.x;z.y=this.p.y;if(z.tick<=0){z.tick=.5;this.enemies.forEach(e=>{if(e.born<=0&&dist(e,z)<z.r)this.heartfireGiftDamage?this.heartfireGiftDamage(e,9+z.lv*4):this.hit(e,9+z.lv*4);});}}
